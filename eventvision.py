# -*- coding: utf-8 -*-
"""
This module contains classes, functions and an example (main) for handling AER
vision data.
"""
import glob
import os
...
class Events(object):
    ...
    def show_em(self):
        ...
        thr = np.rec.array(None,
                           dtype=[('valid', np.bool_), ('low', np.uint64),
                                  ('high', np.uint64)],
        ...
        def show_em_frame(_frame_data):
            """Prepare and show a single frame of em data to be shown"""
            for datum in np.nditer(_frame_data):
        ...
        while frame_start < t_max:
            ...
            frame_data = self.data[
                (self.data.ts >= frame_start) & (self.data.ts < frame_end)]
        ...
        while frame_start < t_max:
            ...
            if frame_data.size > 0:
                ...
                for datum in np.nditer(frame_data):
                    td_img[datum['y'].item(0), datum['x'].item(0)] = datum[
                        'p'].item(0)
                # print 'prepare td frame by iterating events took %s seconds'
                # %em_playback_timer.secs

                td_img = np.piecewise(td_img, [td_img == 0, td_img == 1,
                                               td_img == 128], [0, 255, 128])
    ...
    def filter_td(self, us_time):
        """Generate a filtered set of event data. Does not modify instance
        data Uses a background activity filter on the events, such that only
        events which are correlated with a neighbouring event within
        'us_time' microseconds will be allowed through the filter. us_time:
        microseconds
        """
        max_x = self.width - 1
        max_y = self.height - 1
        ...
        with timer.Timer() as ref_timer:
            for datum in np.nditer(self.data):
                ...
                if x_prev != datum_x | y_prev != datum_y | p_prev != datum_p:
                    ...
                    t0_temp = t0[min_x_sub:(max_x_sub + 1),
                                 min_y_sub:(max_y_sub + 1)]
                ...
                p_prev = datum_p
                i = i + 1
        print('filtering out {} events took {} seconds'.format(
            len(valid_indices) - np.sum(valid_indices), ref_timer.secs))
        return self.data[valid_indices.astype('bool')]

    def sort_order(self):
        """Generate data sorted by ascending ts Does not modify instance
        data Will look through the struct events, and sort all events by the
        field 'ts'. In other words, it will ensure events_out.ts is
        monotonically increasing, which is useful when combining events from
        multiple recordings.
        """
    ...
    def extract_roi(self, top_left, size, is_normalize=False):
        ...
        Does not modify instance data
        Generates a set of td_events which fall into a rectangular region of
        interest with
        top left corner at 'top_left' and size 'size'
        top_left: [x: int, y: int]
        size: [width, height]
        is_normalize: bool. If True, x and y values will be normalized to the
        cropped region
        """
        min_x = top_left[0]
        min_y = top_left[1]
        ...
        extracted_data = self.data[
            (self.data.x >= min_x) & (self.data.x < max_x) & (
                    self.data.y >= min_y) & (self.data.y < max_y)]
    ...
    def write_j_aer(self, filename, chip_size=None):
        """
        ...
        if chip_size is not None:
            y = chip_size[0] - self.data.y - 1
            x = chip_size[1] - self.data.x - 1
        else:
            y = self.data.y
            x = self.data.x
        y_shift = 22 + 32
        ...
        vector_all = np.array(y_final + x_final + p_final + ts_final,
                              dtype=np.uint64)
        aedat_file = open(filename, 'wb')

        version = '2.0'
        aedat_file.write(bytes('#!AER-DAT{0}\r\n'.format(version), 'UTF-8'))
        aedat_file.write(b'# This is a raw AE data file - do not edit\r\n')
        aedat_file.write(b'# Data format is int32 address, int32 timestamp '
                         b'(8 bytes total), repeated for each event\r\n')
        aedat_file.write(b'# Timestamps tick is 1 us\r\n')
        aedat_file.write(bytes(
            '# created {} {} by the Python function \"write2jAER\"\r\n'.format(
                time.strftime("%d/%m/%Y"), time.strftime("%H:%M:%S")),
            'UTF-8'))
        aedat_file.write(b'# This function fakes the format of DAVIS640 to '
                         b'allow for the full ATIS address space to be used '
                         b'(304x240)\r\n')
...
def present_checkerboard(num_squares):
    """
    Presents a checkerboard pattern of size num_squares*num_squares on the
    screen. The function will automatically detect the screen size in pixels
    and assume a resolution of 96 dpi to provide the square size in mm.
    """
    ...
    figure_border_size = 30
    # leave space of 100 pixels on each side of the axes for the figure
    # controls etc
    ...
    square_size_pixels = int(
        min(screen_size_pixels - 2 * figure_border_size) / (num_squares + 2))

    image_border_size = np.array([1, 2])
    image_border_size[0] = (screen_size_pixels[0] - figure_border_size * 2 -
                            square_size_pixels * num_squares) / 2
    image_border_size[1] = (screen_size_pixels[1] - figure_border_size * 2 -
                            square_size_pixels * num_squares) / 2
    ...
    image_inner_dim = num_squares * square_size_pixels
    # the dimenstion of the inside of the image (not including the border)

    # Create a black image to fit both the checkerboard and the image border
    img_template = np.ones((image_inner_dim + 2 * image_border_size[1],
                            image_inner_dim + 2 * image_border_size[0]))
    ...
    print('Warning: Do not resize the checkerboard image window! It has been '
          'shown on the screen at a specific size which must be known for '
          'calibration')
...
def auto_calibrate(num_squares, square_size_mm, scale, image_directory,
                   image_format):
    """
    Will read in images of extension 'image_format' from the directory
    'image_directory' for calibration. Each image should contain a
    checkerboard with 'num_squares'*'num_squares' squares, each of size
    'squareSize_mm'. 'scale' is an optional argument to rescale images
    before calibration because ATIS/DVS have very low resolution and
    calibration algorithms are used to handling larger images (use 4)
    """
    ...
    objp[:, :2] = np.mgrid[0:(num_squares - 1), 0:(num_squares - 1)].T.reshape(
        -1, 2) * square_sidelength
    ...
    for fname in images:
        ...
        # cv2.INTER_CUBIC)
        img = cv2.resize(img_original, None, fx=scale, fy=scale,
        ...
        keypressed = 0
        while keypressed != 13:
            ret, gray_threshold = cv2.threshold(gray, threshold, 255,
        ...
        ret, corners = cv2.findChessboardCorners(
            gray, ((num_squares - 1), (num_squares - 1)),
        ...
        if ret:
            ...
            corners2 = cv2.cornerSubPix(gray, corners, (4, 4), (-1, -1),
                                        criteria)
            imgpoints.append(corners2 / scale)

            # Draw and display the corners
            img = cv2.drawChessboardCorners(img, (
                (num_squares - 1), (num_squares - 1)), corners2, ret)
            cv2.imshow('img', img)
            cv2.waitKey(0)
    ...
    ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(
        objpoints, imgpoints, gray_original.shape[::-1], None, None)
...
def read_aer(filename):
    """Reads in the ATIS file specified by 'filename' and returns the TD and EM
    events.
    This only works for ATIS recordings directly from the GUI.
    If you are working with the N-MNIST or N-CALTECH101 datasets, use
    read_dataset(filename) instead
    """
    ...
    with timer.Timer() as read_aer_timer:
        ...
        # 0
        all_ts2 = raw_data[0::4] | (
                (raw_data[1::4] & 31) << 8)  # bit 4 downto 0
    ...
    print('=> Parsing .val data took {} s'.format(read_aer_timer.secs))
    ...
    print('=> Processing .val data new way took {} s'.format(
        read_aer_timer.secs))
    ...
    # It appears that the polarity needs to be flipped (when results are
    # compared with Matlab output). Change the polarity: 0 events become 1
...
def read_bin_linux(filename):
    ...
    If working with N-MNIST or N-CALTECH101 datasets, use
    ...
    while buffer_location < len(raw_data):
        ...
        evt_type = raw_data[
                   buffer_location:(buffer_location + 8 * num_events):8]
        evt_subtype = raw_data[(buffer_location + 1):(
                buffer_location + 8 * num_events + 1):8]
        y = raw_data[
            (buffer_location + 2):(buffer_location + 8 * num_events + 2):8]
        x = ((raw_data[(buffer_location + 5):(
                buffer_location + 8 * num_events + 5):8].astype(
            np.uint16) << 8) + (raw_data[(buffer_location + 4):(
                buffer_location + 8 * num_events + 4):8]))
        ts = ((raw_data[(buffer_location + 7):(
                buffer_location + 8 * num_events + 7):8].astype(
            np.uint32) << 8) + (raw_data[(buffer_location + 6):(
                buffer_location + 8 * num_events + 6):8]))
        ...
        TD_indices[start_evt_ind:(start_evt_ind + num_events)][
            locations] = True
...
def main():
    ...
    from neuro_dataset import stabilize, apply_tracking3

    path = '/mnt/2646BAF446BAC3B9/Datasets/N-MNIST'
    for phase in ['Test']:  #, 'Train']:
        print(phase)
        for digit in os.listdir(os.path.join(path, 'ATIS', phase)):
            print(digit)
            target_path = os.path.join(path, 'DVS2', phase, digit)
            if not os.path.exists(target_path):
                os.makedirs(target_path)
            for sample in os.listdir(os.path.join(path, 'ATIS', phase, digit)):
                td = read_dataset(os.path.join(path, 'ATIS', phase, digit,
                                               sample))
                # td.show_td(100)
                # td.data = stabilize(td)
                td.data = apply_tracking3(td)
                td.data = td.extract_roi([0, 0], [28, 28], True)
                # td.data = td.apply_refraction(1)
                # td.data = td.filter_td(1000)
                # td.show_td(100)
                sample = os.path.splitext(sample)[0]
                td.write_j_aer(os.path.join(target_path, sample + '.aedat'))
