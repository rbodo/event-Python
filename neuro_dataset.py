"""This module contains classes, functions and a program (main) for reading
neuromorphic datasets, processing the data and then saving into a caffe lmdb"""
import glob
import math
import os
...
def stabilize(td):
    """Compensate for motion of the ATIS sensor during recording of the
...
def apply_tracking1(td, alpha=0.98, threshold=-1):
    """Alternative to stabilization. Compensate for motion of a single "object"
    ...
    2: If the distance is less than a threshold then update the tracker
    location using
    3: tracker_location = tracker_location*alpha + event_location*(1-alpha)
        
    You may find the tracker is quite erratic because it moves with every
    incoming event. It may be a good idea to smooth the motion somewhat which
    would be another step.%

    td: eventvision.Events
    alpha: alpha is a number between 0 and 1. Typically quite high. Default 0.9
    threshold: distance in pixels for the tracker to be updated.
    Default = 0.5 * height of td
    """
    assert (alpha >= 0)
    assert (alpha <= 1)
    mix = 1 - alpha
...
def apply_tracking2(td, num_spikes=20, alpha=0.5, threshold=-1):
    ...
    Alternative to stabilization. Compensate for motion of a single "object" by
    tracking its movement every "num_spike" spikes
    ...
    threshold: distance in number of pixels. Spikes within this distance shall
    be included in the tracker location computation. It is half of the event
    height by default.
    """
    assert (alpha >= 0)
    assert (alpha <= 1)
...
def apply_tracking3(td, time_us=1000, alpha=0.7, threshold=-1):
    """Work as well as tracking 1, but faster
    Alternative to stabilization. Compensate for motion of a single "object"
    by tracking its movement every time_us microseconds
    1: Filter spikes far from the tracker
    2: tracker location = tracker_location*alpha + filtered_average*(1-alpha)
...
def make_td_images(td, num_spikes, step_factor=1):
    """Generate set of images from the Temporal Difference (td) events by
    reading a number of unique spikes
    td is read from a binary file (refer to eventvision.Readxxx functions)
    td: eventvision.Events
    num_spikes: number of unique spikes to accumulate before generating an
    image
    step_factor: proportional amount to shift before generating the next image.
        ...
        0.6 would result in the next image overlapping with 40% of the previous
         image
...
def make_td_probability_image(td, skip_steps=0, is_normalize=False):
    """Generate image from the Temporal Difference (td) events with each pixel
    value indicating probability of a spike within a 1 millisecond time step.
    0 = 0%. 255 = 100%
    td is read from a binary file (refer to eventvision.Readxxx functions)
    td: eventvision.Events
    skip_steps: number of time steps to skip (to allow tracker to init to a
    more correct position)
    is_normalize: True to make the images more obvious (by scaling max
    probability to pixel value 255)
    """
    assert isinstance(td, ev.Events)
...
def prepare_n_mnist(filename, is_filter, num_spikes, step_factor=1):
    ...
    is_filter: True if median filtering should be applied to the constructed
    image
    ...
    step_factor: proportional amount to shift before generating the next image
        ...
        0.6 would result in the next image overlapping with 40% of the previous
         image
    returns: list of images, where each image is a 2d numpy array
    (height, width)
    """
...
def prepare_n_mnist_continuous(filename, is_filter, is_normalize=False):
    ...
    is_filter: True if median filtering should be applied to the constructed
    image
    is_normalize: If True, the probabilities will be normalized to make the
    image more obvious
...
def add_images_to_dataset(image_dataset, images, add_index, label, width,
                          height):
...
def save_to_lmdb(image_dataset, output_lmdb, is_float_data):
    ...
    for i in shuffled_indices:
        ...
        if is_float_data:
            ...
            datum.data = image[
                'image_data'].tobytes()  # or .tostring() if numpy < 1.9
...
def save_to_mat(image_dataset, output_mat):
    ...
    # Numpy Array which will be written into output matrix (Pre-allocate
    # memory)
    # Assumes image_dataset[i]['image_data'] is of the format = [height, width]
    # np.uint8 is sufficient if space is an issue
    images = np.zeros((num_images, num_features), dtype=np.float)
    ...
    savemat(output_mat, {'labels': labels, 'data': images}, appendmat=True,
            do_compression=True)


def generate_nmnist_dataset(initial_size, input_dir, num_spikes, step_factor):
    """Parse the specified directory containing nmnist files to generate an
    image dataset
    ...
    input_dir: input directory.
        Should contain folders 0 to 9, each containing a set of bin files
        (n mnist recordings)
    num_spikes: number of unique spikes per image
    step_factor: proportional amount to shift before generating the next image
        1 would result in no overlapping events between images
...
def generate_nmnist_continuous_dataset(initial_size, input_dir):
    """Parse the specified directory containing nmnist files to generate an
    image dataset meant for training
    initial_size: initial size of the image dataset.
        Set this to an appropriately high value to avoid expensive reallocation
    input_dir: input directory.
        Should contain folders 0 to 9, each containing a set of bin files
